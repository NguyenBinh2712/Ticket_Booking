...
public interface RevenueTransactionRepository extends JpaRepository<RevenueTransaction, Long> {
    ...
    @Query("SELECT rt FROM RevenueTransaction rt " +
            "WHERE rt.status = 'PENDING' AND rt.settlement IS NULL " +
            "AND rt.booking.payment.paidAt >= :from AND rt.booking.payment.paidAt < :to")
    List<RevenueTransaction> findUnsettledInPeriod(@Param("from") LocalDate from, @Param("to") LocalDate to);

    List<RevenueTransaction> findBySettlement(Settlement settlement);
    @Query("SELECT COALESCE(SUM(rt.totalAmount), 0) FROM RevenueTransaction rt WHERE rt.status != 'REVERSED'")
    BigDecimal sumTotalRevenue();

    @Query("SELECT COALESCE(SUM(rt.adminAmount), 0) FROM RevenueTransaction rt WHERE rt.status != 'REVERSED'")
    BigDecimal sumAdminCommission();

    long countByStatusNot(TransactionStatus status);

    @Query(
            value = "SELECT DATE_FORMAT(created_at, '%Y-%m') as month, SUM(total_amount) as total " +
            "FROM revenue_transactions WHERE status != 'REVERSED' " +
